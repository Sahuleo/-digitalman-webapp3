...
function tryLogin(){
  ...
  const err=dom('#loginError'); err.style.display='none'; err.textContent='';
  const user=state.data.employees.find(e=> e.username===u);
  if(!user){ err.textContent='کاربر پیدا نشد.'; err.style.display='block'; return; }
  if(!p){ err.textContent='PIN را وارد کنید.'; err.style.display='block'; return; }
  if(String(user.pin)!==String(p)){ err.textContent='PIN نادرست است.'; err.style.display='block'; return; }
  state.user=user; dom('#pinInput').value=''; render();
}
function logout(){ state.user=null; render(); }
function render(){
  const logged=!!state.user;
  dom('#login').style.display= logged? 'none':'';
  dom('#dashboard').style.display= logged? '':'';
  dom('#tabs').style.display= logged? 'flex':'none';
  dom('#btnLogout').style.display= logged? '':'';
  ...
  const adminTab=dom('#adminTab'); adminTab.style.display= isAdmin? '':'';
  if(isAdmin){
...
function attach(){
  ...
  document.querySelectorAll('.tab').forEach(t=> t.addEventListener('click', ()=>{
    ...
    dom('#dashboard').style.display= id==='dashboard'? '':'';
    dom('#admin').style.display= id==='admin'? '':'';
  }));
}
